...
WHERE namespace.nspname='public'
  AND procedure.proname IN ('controlled_submit_transfer_v1','controlled_post_funding_v1')
  AND procedure.prosecdef
  ...
  AND NOT EXISTS (SELECT 1 FROM aclexplode(procedure.proacl) acl WHERE acl.grantee=0 AND acl.privilege_type='EXECUTE')`).Scan(&controlledFinancialFunctions); err != nil {
	...
	if compositeUniqueKeys != 2 || validatedCompositeForeignKeys != 4 || hardenedHydrators != 2 || hardenedSemanticFunctions != 2 || controlledFinancialFunctions != 2 || semanticTriggers != 4 {
