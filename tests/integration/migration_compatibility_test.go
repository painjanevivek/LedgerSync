...
func TestMigrationsAreForwardCompatibleAndPreserveExistingReadContracts(t *testing.T) {
	...
	for _, table := range []string{"accounts", "account_credit_permissions", "ledger_postings", "outbox_events", "reconciliation_runs", "reconciliation_mismatches", "delivery_attempts", "tenant_transfer_policies", "api_rate_limit_windows", "account_opening_balances"} {
		var exists bool
