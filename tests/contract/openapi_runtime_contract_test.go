...
func TestOpenAPIContainsEveryRegisteredMVPRouteAndLosslessBoundaries(t *testing.T) {
	...
	for _, route := range []string{"/accounts:", "/me/accounts:", "/accounts/{accountId}:", "/accounts/{accountId}/balance:", "/accounts/{accountId}/transactions:", "/transfers:", "/transfers/{transferId}:", "/investigation/search:", "/investigation/related/{recordType}/{recordId}:", "/investigation/saved-views:", "/investigation/saved-views/{savedViewId}:", "/funding-requests:", "/funding-events:", "/funding-events/{fundingEventId}:", "/funding-events/{fundingEventId}/approve:", "/funding-events/{fundingEventId}/reject:", "/funding-events/{fundingEventId}/post:", "/funding-events/{fundingEventId}/compensations:", "/funding-events/{fundingEventId}/reconciliation:", "/reconciliation/runs:", "/reconciliation/runs/{runId}:"} {
