...
func TestDatabaseRoleContractKeepsSupportReadOnlyAndBreakGlassGrantless(t *testing.T) {
	...
	for _, table := range []string{"api_rate_limit_windows", "journal_transactions", "ledger_postings", "delivery_attempts", "webhook_delivery_jobs", "webhook_endpoint_verification_jobs", "delivery_replay_actions", "reconciliation_runs", "reconciliation_mismatches", "investigation_saved_views", "partner_credential_events", "developer_webhook_endpoints", "developer_webhook_events", "bff_actor_assertion_replays"} {
