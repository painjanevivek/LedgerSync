...
func TestDatabaseRoleContractKeepsSupportReadOnlyAndBreakGlassGrantless(t *testing.T) {
	...
	for _, table := range []string{"api_rate_limit_windows", "journal_transactions", "ledger_postings", "delivery_attempts", "delivery_replay_actions", "reconciliation_runs", "reconciliation_mismatches", "partner_credential_events", "developer_webhook_endpoints", "developer_webhook_events"} {
