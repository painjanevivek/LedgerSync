...
func TestDatabaseRoleContractKeepsSupportReadOnlyAndBreakGlassGrantless(t *testing.T) {
	...
	for _, table := range []string{"api_rate_limit_windows", "journal_transactions", "ledger_postings", "delivery_attempts", "webhook_delivery_jobs", "webhook_endpoint_verification_jobs", "delivery_replay_actions", "reconciliation_runs", "reconciliation_mismatches", "investigation_saved_views", "investigation_workspaces", "investigation_workspace_references", "partner_credential_events", "developer_webhook_endpoints", "developer_webhook_events", "bff_actor_assertion_replays"} {
		if !strings.Contains(contract, table) {
			t.Errorf("API investigation/rate-limit contract omits %s", table)
		}
	}
	for _, marker := range []string{
		"controlled_submit_transfer_v1(uuid,text,uuid,uuid,bigint,text,text,bytea,uuid)",
