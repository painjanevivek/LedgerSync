...
| M04 | Guided first-run journey | `ACTIVE` | M02, M03 | Product/web | Existing durable-evidence local guide and local-product Phase 8 evidence | Add server-owned resumable preferences and context-aware next actions; implement every safe pre-funding step now and bind the complete funding-to-backup journey as M05 becomes available. |
