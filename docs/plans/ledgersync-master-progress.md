...
| M07 | API-first developer product | `ACTIVE` | M05, M06 | Developer platform | Reviewed OpenAPI 1.11, safe local developer view, exact examples, correlation IDs, idempotency guidance, and bounded private routes | Add credential lifecycle, webhooks, sandbox indicators, compatibility/deprecation policy, complete multi-language examples, bounded bulk provisioning, and reproducible SDK drift enforcement. |
