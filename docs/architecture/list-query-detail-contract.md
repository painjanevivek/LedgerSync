...
| Accounts | Oldest `created_at`, then immutable ID | Exact ID, normalized display-name/external-reference prefix, exact status, exact category | Opaque cursor bound to order; page count is derived; no total | List export is not released because balances are current evidence rather than an immutable ledger extract; an individual account has a bounded exact full-history export independent of directory-page position | Account detail/focus with exact filtered `/accounts` return URL | Converged for the released query surface: strict page/BFF validation, URL-led clear/filter/pagination, count/order labels, exact return context, and full-account history export scope are implemented |
...
| Accounts | `q`, `status`, `category`, `cursor`, optional UI `focus`, BFF-only `limit` | query ≤128 with escaped prefix semantics; allowlisted status/category; cursor ≤512 to match the private account contract; focus is a canonical UUID and does not broaden the private query |
