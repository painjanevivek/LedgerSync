...
| LPC-020 | Phase 2 — account command API and BFF | Additive account mutation routes enforce `accounts:write`, object authorization, strict bounded input, idempotency, optimistic concurrency, no-store, rate/capacity controls, CSRF, fixed origin/host, and response-unknown retry semantics | OpenAPI/contract results, handler/BFF security tests, real BFF-to-PostgreSQL create/retry/conflict evidence, unchanged existing-route proof | `READY` | Implement Phase 2 against the passed LPC-010 account transaction boundary |
