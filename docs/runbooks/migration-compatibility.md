...
Migration `000039_tenant_rls_expand` enables compatibility-mode RLS on 19 protected financial and evidence tables. A present transaction-local `ledgersync.tenant_id` scopes reads and writes; a missing value remains temporarily compatible while workload coverage is observed. Reverse migrations must remove `000039` policies before dropping tenant columns introduced by earlier semantic-key migrations.
