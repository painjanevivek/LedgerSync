...
const relationshipParameters = `relationship_parameters AS (
 SELECT $1::uuid AS tenant_id,$2::text AS actor_id,$3::uuid AS source_id,
  $4::boolean AS accounts,$5::boolean AS transfers,$6::boolean AS funding,
  $7::boolean AS events,$8::boolean AS reconciliation,$9::boolean AS corrections,
  $10::integer AS result_limit WHERE FALSE
)`

const relationshipColumns = `relationships(relationship_type,target_type,target_id,safe_label,status,occurred_at)`

const accountRelationshipsSQL = `WITH ` + relationshipParameters + `, ` + relationshipColumns + ` AS (
...
const transferRelationshipsSQL = `WITH ` + relationshipParameters + `, ` + relationshipColumns + ` AS (
...
const fundingRelationshipsSQL = `WITH ` + relationshipParameters + `, ` + relationshipColumns + ` AS (
...
const eventRelationshipsSQL = `WITH ` + relationshipParameters + `, ` + relationshipColumns + ` AS (
...
const reconciliationRunRelationshipsSQL = `WITH ` + relationshipParameters + `, ` + relationshipColumns + ` AS (
...
const reconciliationMismatchRelationshipsSQL = `WITH ` + relationshipParameters + `, ` + relationshipColumns + ` AS (
...
const correctionRelationshipsSQL = `WITH ` + relationshipParameters + `, ` + relationshipColumns + ` AS (
