...
func TestAllowedSetDropsUnknownRolesAndScopes(t *testing.T) {
	...
	scopes := allowedSet([]string{"accounts:read", "accounts:write", "reconciliation:write", "local:read", "local:write", "events:read", "investigation:read", "investigation:write", "investigation:collaborate", "developer:read", "recovery:read", "exports:read", "explainability:read", "funding:read", "funding:write", "funding:approve", "accounts:all"}, allowedScopes)
	if len(scopes) != 16 || !contains(scopes, "accounts:read") || !contains(scopes, "accounts:write") || !contains(scopes, "reconciliation:write") || !contains(scopes, "local:read") || !contains(scopes, "local:write") || !contains(scopes, "events:read") || !contains(scopes, "investigation:read") || !contains(scopes, "investigation:write") || !contains(scopes, "investigation:collaborate") || !contains(scopes, "developer:read") || !contains(scopes, "recovery:read") || !contains(scopes, "exports:read") || !contains(scopes, "explainability:read") || !contains(scopes, "funding:read") || !contains(scopes, "funding:write") || !contains(scopes, "funding:approve") {
...
func TestBFFActorAssertionRequiresSignedShortLivedActorContext(t *testing.T) {
	...
	if !principal.AuthenticatedAt.Equal(now.Add(-time.Minute)) {
		t.Fatalf("expected signed authentication time, got %s", principal.AuthenticatedAt)
	}
