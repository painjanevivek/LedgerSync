...
export function RecordLink({ href, label }: Readonly<{ href: string; label: string }>) { return <Link className="record-link" href={href}>{label}<ArrowRight aria-hidden="true" /></Link>; }

export function Pagination({ nextCursor, onNext, busy }: Readonly<{ nextCursor?: string; onNext: () => void; busy?: boolean }>) {
  return <div className="pagination"><span>{nextCursor ? "More records are available" : "End of available records"}</span><button className="button secondary" type="button" disabled={!nextCursor || busy} onClick={onNext}>{busy ? "Loading…" : "Load more"}</button></div>;
}
