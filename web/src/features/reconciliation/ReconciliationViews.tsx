...
type ReconciliationViewProps = Readonly<{
  runs: ReconciliationRun[]; detail: ReconciliationRun | null; detailRequested: boolean; error: string | null; loading: boolean; verifiedAt?: string; nextCursor?: string; tenantId: string; csrfToken: string; online: boolean; canWrite: boolean; canExport: boolean; returnTo?: string; onObserved: (run: ReconciliationRun) => void; onRefresh: () => Promise<void>; onMore: () => void;
}>;

export function ReconciliationView({ runs, detail, detailRequested, error, loading, verifiedAt, nextCursor, tenantId, csrfToken, online, canWrite, canExport, returnTo, onObserved, onRefresh, onMore }: ReconciliationViewProps) {
  if (detail) return <>
    <PageHeader eyebrow="Control record / Immutable run" title="Reconciliation detail" description="Result returned by the tenant-authorized PostgreSQL control record."><EvidenceExportControl label="Export run result" subject="reconciliation run" endpoint={`/api/exports/reconciliation.csv?runId=${encodeURIComponent(detail.run_id)}&limit=10000`} scope={`One immutable run · ${detail.run_id}`} filters={[{ label: "Run ID", value: detail.run_id }]} columns="Includes run, mismatch, account, and correlation identifiers when present" online={online} canExport={canExport} /></PageHeader>
  ...
  const latest = runs[0];
  return <>
    <PageHeader eyebrow="Controls / Authoritative ledger" title="Reconciliation" description="Verify whether projected account balances agree with immutable double-entry postings."><button className="button secondary" type="button" disabled={loading || !online} onClick={() => void onRefresh()}>{loading ? "Refreshing results…" : "Refresh results"}</button></PageHeader>
    ...
    {runs.length > 0 && <section className="ledger-section">
      <div className="section-heading"><div><p className="eyebrow">Immutable history</p><h2>Reconciliation runs</h2><p>Each result retains its scope, counts, watermark, and completion time. Starting a new run never replaces prior results.</p></div><EvidenceExportControl label="Export reconciliation results" subject="reconciliation history" endpoint="/api/exports/reconciliation.csv?limit=10000" scope="All authorized reconciliation runs and mismatch details" filters={[]} columns="Includes run, mismatch, account, and correlation identifiers when present" online={online} canExport={canExport} /></div>
      <DataTableRegion label="Reconciliation run history"><table className="data-table"><thead><tr><th>Run ID</th><th>Result</th><th>Scope</th><th>Accounts / postings</th><th>Mismatches</th><th>Completed UTC</th><th>Action</th></tr></thead><tbody>{runs.map((run) => <tr key={run.run_id}><td><CopyControl value={run.run_id} /></td><td><StatusBadge tone={runTone(run.status)}>{runLabel(run.status)}</StatusBadge></td><td>{run.scope}</td><td>{run.checked_account_count} / {run.posting_count}</td><td className="number-cell">{run.mismatch_count}</td><td>{run.status === "running" ? "In progress" : utcDateTime(run.completed_at)}</td><td><RecordLink href={`/reconciliation/${run.run_id}`} label="Open result" /></td></tr>)}</tbody></table></DataTableRegion>
      <Pagination nextCursor={nextCursor} busy={loading} onNext={onMore} />
    </section>}
  </>;
}
