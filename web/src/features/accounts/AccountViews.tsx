...
import { CopyControl, DataTableRegion, EvidenceFreshness, PageHeader, Pagination, RecordLink, StatePanel, StatusBadge } from "@/features/console/components";
...
export function AccountsView({ accounts, selected, detailRequested, balance, transactions, balanceLoading, historyLoading, directoryLoading, balanceError, historyError, balanceVerifiedAt, historyVerifiedAt, directoryVerifiedAt, error, online, filters, nextCursor, historyNextCursor, focusAccountId, tenantId, csrfToken, canWrite, canTransfer, canExport, fundingScopeComplete, detailReturnTo, onRefresh, onApplyFilters, onNext, onHistoryNext, onAccountChanged, onRefreshLifecycleEvidence }: Props) {
  ...
  return <>
    ...
    {!detailRequested && !selected && <>
      ...
      {directoryLoading && accounts.length === 0 ? <StatePanel title="Loading authorized accounts" message="LedgerSync is requesting one bounded page from the authoritative account directory." /> : accounts.length === 0 && !error ? <StatePanel title="No matching accounts" message="Clear or change the filters. LedgerSync does not broaden the authorized account scope." /> : accounts.length > 0 && <section className="ledger-section account-directory" aria-labelledby="accounts-heading" aria-busy={directoryLoading}>
    ...
    {selected && <>
      ...
      <div className="account-detail-grid"><BalanceStatus currency={balance?.currency ?? selected.currency} availableMinor={balance?.available_minor} version={balance?.version} asOf={balance?.as_of} verifiedAt={balanceVerifiedAt} loading={balanceLoading} error={balanceError} /><TransactionLedger transactions={transactions} account={selected} loading={historyLoading} error={historyError} verifiedAt={historyVerifiedAt} nextCursor={historyNextCursor} onNext={onHistoryNext} exportAction={<EvidenceExportControl label="Export ledger evidence" subject="account ledger history" endpoint={`/api/exports/accounts/${encodeURIComponent(selected.account_id)}/transactions.csv?limit=10000`} scope={`One authorized account · ${selected.account_id}`} filters={[]} columns="Includes the full account, transfer, journal, and posting identifiers with exact quoted minor-unit strings" online={online} canExport={canExport}/>}/></div>
