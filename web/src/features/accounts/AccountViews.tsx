...
export function AccountsView({ accounts, selected, detailRequested, balance, transactions, balanceLoading, historyLoading, directoryLoading, balanceError, historyError, balanceVerifiedAt, historyVerifiedAt, directoryVerifiedAt, error, online, filters, nextCursor, historyNextCursor, focusAccountId, tenantId, csrfToken, canWrite, canTransfer, canExport, fundingScopeComplete, detailReturnTo, onRefresh, onHistoryNext, onRefreshBalance, onRefreshHistory, onAccountChanged, onRefreshLifecycleEvidence }: Props) {
  ...
  return <>
    ...
    {!detailRequested && !selected && <>
      ...
      {directoryLoading && accounts.length === 0 ? <StatePanel title="Loading authorized accounts" message="LedgerSync is checking which accounts are available to you." /> : accounts.length === 0 && !error ? <StatePanel title={filters.query || filters.status || filters.category ? "No matching accounts" : "No accounts yet"} message={filters.query || filters.status || filters.category ? "Clear or change the filters. LedgerSync does not broaden the authorized account scope." : "Create your first account to begin the ledger. It opens at an exact zero balance."} action={!filters.query && !filters.status && !filters.category && canWrite ? <Link className="button primary" href="/accounts/new">Create your first account</Link> : undefined} /> : null}
      {accounts.length > 0 && mode === "simple" && <section className="simple-record-section" aria-labelledby="accounts-heading" aria-busy={directoryLoading}>
        <div className="simple-section-heading"><div><h2 id="accounts-heading">Your accounts</h2><p>{accounts.length} {accounts.length === 1 ? "account is" : "accounts are"} available on this page.</p></div></div>
        <ul className="simple-record-list">{accounts.map((account) => <li key={account.account_id} data-tone={account.status === "active" ? "positive" : account.status === "frozen" ? "warning" : "neutral"}><article><div className="simple-record-main"><div><strong>{accountLabel(account)}</strong><span>{account.status === "active" ? "Ready to use" : account.status === "frozen" ? "Temporarily unavailable" : "Closed"}</span></div><Money currency={account.currency} minorUnits={account.available_minor} /></div><p>{account.category ? `${account.category.replaceAll("_", " ")} account` : "Account category not set"} · <RelativeTime value={account.as_of} /></p><div className="simple-record-actions"><Link id={`account-link-${account.account_id}`} className="button secondary" href={accountDetailHref(account.account_id, filters)}>View account</Link><TechnicalDetails><RecordIdentity label="Account reference" value={account.account_id} /><p>Balance version {account.version}.</p></TechnicalDetails></div></article></li>)}</ul>
        {nextCursor && !directoryLoading && <div className="pagination"><span>More accounts are available</span><Link className="button secondary" href={accountDirectoryURL({ ...filters, cursor: nextCursor })}>Next page</Link></div>}
      </section>}
      {accounts.length > 0 && mode === "expert" && <section className="ledger-section account-directory" aria-labelledby="accounts-heading" aria-busy={directoryLoading}>
      ...
      {mode === "expert" && (accounts.length > 0 || Boolean(filters.query || filters.status || filters.category)) && <>
    ...
    {selected && <>
      {mode === "simple" && <TechnicalDetails summary="View account details"><RecordIdentity label="Account reference" value={selected.account_id} /><p>External reference: {selected.external_reference || "Not set"}.</p><p>Category: {selected.category?.replaceAll("_", " ") || "Not classified"}.</p></TechnicalDetails>}
      {mode === "expert" && <section className="identity-strip"><div><span>Account ID</span><CopyControl value={selected.account_id} /></div><div><span>External reference</span><strong>{selected.external_reference || "Not set"}</strong></div><div><span>Category</span><strong>{selected.category?.replaceAll("_", " ") || "Unclassified"}</strong></div><div><span>Status</span><StatusBadge tone={tone(selected.status)}>{selected.status}</StatusBadge></div></section>}
