  ...
  return <>
    ...
    {!detailRequested && !selected && !error && <>
      ...
      {directoryLoading && accounts.length === 0 ? <StatePanel title="Loading authorized accounts" message="LedgerSync is requesting one bounded page from the authoritative account directory." /> : accounts.length === 0 ? <StatePanel title="No matching accounts" message="Clear or change the filters. LedgerSync does not broaden the authorized account scope." /> : <section className="ledger-section account-directory" aria-labelledby="accounts-heading" aria-busy={directoryLoading}>
        ...
        <div className="data-table-wrap" role="region" aria-label="Authorized account comparison" tabIndex={0}><table className="data-table"><thead><tr><th>Account</th><th>Category</th><th>Status</th><th>Available balance</th><th>Version / as of</th><th>Action</th></tr></thead><tbody>{accounts.map((account) => <tr key={account.account_id}><td><strong>{accountLabel(account)}</strong><code>{account.account_id}</code></td><td>{account.category?.replaceAll("_", " ") ?? "Unclassified"}</td><td><StatusBadge tone={tone(account.status)}>{account.status}</StatusBadge></td><td className="number-cell">{formatMinorUnits(account.currency, account.available_minor)}</td><td><code>v{account.version}</code><small>{utcDateTime(account.as_of)}</small></td><td><RecordLink id={`account-link-${account.account_id}`} href={accountDetailHref(account.account_id, filters)} label="Open account" /></td></tr>)}</tbody></table></div>
        <div className="record-card-list">{accounts.map((account) => <article className="record-card" key={account.account_id}><div><StatusBadge tone={tone(account.status)}>{account.status}</StatusBadge><span>{account.category?.replaceAll("_", " ")}</span></div><h3>{accountLabel(account)}</h3><strong>{formatMinorUnits(account.currency, account.available_minor)}</strong><CopyControl value={account.account_id} label={`Copy ${accountLabel(account)} ID`} /><small>Version {account.version} · {utcDateTime(account.as_of)}</small><RecordLink href={accountDetailHref(account.account_id, filters)} label="Open account" /></article>)}</div>
