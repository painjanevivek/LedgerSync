...
import { RecordLink, StatePanel, StatusBadge } from "@/features/console/components";
...
export function TransactionLedger({ transactions, account, loading, error }: Props) {
  return <section className="ledger-section" aria-labelledby="transactions-heading" aria-busy={loading}>
    ...
    {loading && <StatePanel title="Loading ledger history" message="Reading immutable postings from the authorized account scope." />}
    {!loading && error && <StatePanel kind="error" title="Ledger history unavailable" message={error} />}
    ...
    {!loading && !error && transactions.length > 0 && <div className="ledger-list">{transactions.map((transaction) => <article className="ledger-row" key={`${transaction.transfer_id}-${transaction.direction}`}>
