  ...
  return <>
    ...
    {evidence && <section className="operations-status" aria-labelledby="operations-status-heading" aria-busy={loading}>
      ...
      <div className="truth-ledger" role="region" aria-label="Ordered local truth domains">
        <DisclosureSection id="local-postgres-status" title="PostgreSQL ledger" summary={`Financial authority · ${stateLabel(evidence.financial_authority.postgres.state)}`} attention={evidence.financial_authority.postgres.state !== "reachable"} defaultOpen={evidence.financial_authority.postgres.state !== "reachable"}><article className="truth-lane financial-truth"><div className="truth-order">01</div><Database aria-hidden="true"/><div className="truth-copy"><p className="eyebrow">Financial authority</p><h3>PostgreSQL ledger</h3><p>Balances, postings, and reconciliation evidence are authoritative only here.</p></div><StatusBadge tone={stateTone(evidence.financial_authority.postgres.state)}>{stateLabel(evidence.financial_authority.postgres.state)}</StatusBadge><dl><div><dt>Schema version</dt><dd><code>{evidence.financial_authority.postgres.schema_version ?? "Not available"}</code></dd></div><div><dt>Latest reconciliation</dt><dd>{stateLabel(evidence.financial_authority.latest_reconciliation.state)}{evidence.financial_authority.latest_reconciliation.status ? ` · ${stateLabel(evidence.financial_authority.latest_reconciliation.status)}` : ""}</dd></div></dl>{evidence.financial_authority.latest_reconciliation.run_id && <RecordLink href={`/reconciliation/${encodeURIComponent(evidence.financial_authority.latest_reconciliation.run_id)}`} label="Open reconciliation evidence" />}</article></DisclosureSection>
        <DisclosureSection id="local-outbox-status" title="Transactional outbox" summary={`Downstream delivery · ${stateLabel(evidence.delivery_cache.outbox.state)}`} attention={evidence.delivery_cache.outbox.state !== "reachable" || evidence.delivery_cache.outbox.worker_progress === "stalled" || evidence.delivery_cache.outbox.dead_count !== "0"}><article className="truth-lane delivery-truth"><div className="truth-order">02</div><Lightning aria-hidden="true"/><div className="truth-copy"><p className="eyebrow">Downstream delivery</p><h3>Transactional outbox</h3><p>Delivery can lag or fail after a financial command has already committed.</p></div><StatusBadge tone={stateTone(evidence.delivery_cache.outbox.state)}>{stateLabel(evidence.delivery_cache.outbox.state)}</StatusBadge><dl><div><dt>Pending</dt><dd>{evidence.delivery_cache.outbox.pending_count ?? "Not available"}</dd></div><div><dt>Dead</dt><dd>{evidence.delivery_cache.outbox.dead_count ?? "Not available"}</dd></div><div><dt>Worker progress</dt><dd>{stateLabel(evidence.delivery_cache.outbox.worker_progress)}</dd></div><div><dt>Oldest pending</dt><dd>{utc(evidence.delivery_cache.outbox.oldest_pending_at)}</dd></div></dl><RecordLink href="/events" label="Investigate delivery events" /></article></DisclosureSection>
        <DisclosureSection id="local-redis-status" title="Redis cache" summary={`Disposable acceleration · ${stateLabel(evidence.delivery_cache.redis.state)}`} attention={evidence.delivery_cache.redis.state !== "reachable"}><article className="truth-lane cache-truth"><div className="truth-order">03</div><HardDrives aria-hidden="true"/><div className="truth-copy"><p className="eyebrow">Disposable acceleration</p><h3>Redis cache</h3><p>A cache outage may slow reads. It is never evidence that PostgreSQL money changed.</p></div><StatusBadge tone={stateTone(evidence.delivery_cache.redis.state)}>{stateLabel(evidence.delivery_cache.redis.state)}</StatusBadge><dl><div><dt>Role</dt><dd>Disposable cache</dd></div><div><dt>Fallback</dt><dd>Version-checked authoritative read</dd></div></dl></article></DisclosureSection>
      </div>
