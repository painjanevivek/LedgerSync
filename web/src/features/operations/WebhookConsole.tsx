...
import { WebhookEndpointDetailView, WebhookEndpointListView, type WebhookFilters } from "@/features/operations/WebhookViews";
...
const emptyFilters: WebhookFilters = {};
...
export function WebhookConsole({ endpointId, filters = emptyFilters, returnTo }: Readonly<{ endpointId?: string; filters?: WebhookFilters; returnTo?: string }>) {
  ...
  useEffect(() => {
    if (!session || !online || !hasScope("webhooks:read")) return;
  ...
  }, [endpointId, hasScope, loadDetail, loadList, online, session]);
  ...
  return <ConsoleShell section="events" tenantLabel={session.tenant_label ?? "Ledger tenant"} tenantMeta={session.tenant_id} environmentLabel={session.environment === "local" ? "Local workspace" : "Verified production"} operatorLabel={session.operator_label ?? session.subject_id} operatorMeta={session.environment === "local" ? "This workstation" : "Authorized operator"}>
    ...
    {endpointId ? (
      ...
      <WebhookEndpointListView items={items} filters={filters} nextCursor={nextCursor} verifiedAt={verifiedAt} loading={loading} error={error} online={online} canRead={canRead} onRefresh={() => void loadList()}/>
    )}
    <ConsoleFooter/>
  </ConsoleShell>;
}
