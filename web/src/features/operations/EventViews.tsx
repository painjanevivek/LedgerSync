...
export function EventsListView({ events, filters, nextCursor, verifiedAt, loading, error, online, canRead, onRefresh }: Readonly<{ events: DeliveryEvent[]; filters: EventFilters; nextCursor?: string; verifiedAt?: string; loading: boolean; error: string | null; online: boolean; canRead: boolean; onRefresh: () => void }>) {
  ...
  const attentionCount = events.filter((event) => event.state === "dead" || event.state === "retrying").length;
  ...
  return <>
    ...
    {attentionCount > 0 && <NextBestAction attention eyebrow="Delivery health" title={`${attentionCount} failed or retrying event${attentionCount === 1 ? "" : "s"} need attention`} message="Financial posting remains separate. Inspect these delivery records before reviewing healthy recent history." />}
    <AdvancedFilterPanel id="event-advanced-filters" title="Advanced event filters" activeCount={activeFilterCount}>
...
export function EventDetailView({ event, verifiedAt, loading, error, online, canRead, returnTo, onRefresh }: Readonly<{ event: DeliveryEventDetail | null; verifiedAt?: string; loading: boolean; error: string | null; online: boolean; canRead: boolean; returnTo?: string; onRefresh: () => void }>) {
  ...
  return <>
    ...
    {event && <article className="event-detail-document" aria-labelledby="event-detail-heading" aria-busy={loading}>
      ...
      <DisclosureSection id="event-technical-identifiers" title="Technical event identifiers" summary="Aggregate version, timestamps, correlation, and bounded error code." priority="advanced" lazy><dl className="evidence-list"><div><dt>Aggregate</dt><dd>{event.aggregate_type} · version {event.aggregate_version}</dd></div><div><dt>Aggregate ID</dt><dd><CopyControl value={event.aggregate_id} label="Copy aggregate ID" /></dd></div><div><dt>Attempt count</dt><dd>{event.attempt_count}</dd></div><div><dt>Occurred</dt><dd>{utc(event.occurred_at)}</dd></div><div><dt>Available</dt><dd>{utc(event.available_at)}</dd></div>{event.correlation_id && <div><dt>Correlation ID</dt><dd><CopyControl value={event.correlation_id} label="Copy correlation ID" /></dd></div>}{event.last_error_code && <div><dt>Last bounded error code</dt><dd><code>{event.last_error_code}</code></dd></div>}</dl></DisclosureSection>
    ...
    {event && <DisclosureSection id="event-related-evidence" title="Related investigation evidence" summary="Open additional authorized records linked to this event." lazy><RelatedEvidenceRail sourceType="event" sourceId={event.event_id} /></DisclosureSection>}
  </>;
}
