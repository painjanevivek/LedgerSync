...
import { FundingDetailView, FundingListView } from "@/features/funding/FundingViews";
...
export function FundingConsole({ fundingEventId }: Readonly<{ fundingEventId?: string }>) {
  ...
  return <ConsoleShell section="funding" tenantLabel={session.tenant_label ?? "Ledger tenant"} tenantMeta={session.tenant_id} environmentLabel={session.environment === "demo" ? "Isolated demo" : "Verified production"} operatorLabel={session.operator_label ?? session.subject_id} operatorMeta={session.environment === "demo" ? "Non-production data" : "Authorized finance operator"} preview={session.environment === "demo"} onSignOut={() => void signOut()}>
