...
function fundingStatusLabel(status: FundingStatus): string {
  if (status === "requested") return "Waiting for review";
  if (status === "approved") return "Ready to add";
  if (status === "posted") return "Money added";
  if (status === "compensated") return "Record corrected";
  return "Not approved";
}
...
export function FundingListView({ events, accounts, filters, nextCursor, verifiedAt, loading, error, online, canWrite, onApplyFilters, onClearFilters, onOpenRequest, onRefresh }: Readonly<{
  ...
  return <>
    ...
    {loading && events.length === 0 ? <StatePanel title="Loading money added" message="Checking your records now. An empty result will appear only after the check finishes." /> : events.length === 0 && !error ? <StatePanel title="No money has been added yet" message="When money is confirmed outside LedgerSync, add its reference number and supporting document here. It will be checked before your balance changes." action={canWrite ? <button className="button primary" type="button" onClick={onOpenRequest}>Add money</button> : undefined} /> : null}
    {events.length > 0 && mode === "simple" && <section className="simple-record-section" aria-labelledby="funding-ledger-heading" aria-busy={loading}><div className="simple-section-heading"><div><h2 id="funding-ledger-heading">Money added</h2><p>{events.length} {events.length === 1 ? "record is" : "records are"} shown on this page.</p></div></div><ul className="simple-record-list">{events.map((event) => <li key={event.funding_event_id} data-tone={event.status === "rejected" ? "danger" : event.status === "requested" ? "warning" : "positive"}><article><div className="simple-record-main"><div><strong>{fundingStatusLabel(event.status)}</strong><span>{labels.get(event.destination_account_id) ?? "Authorized account"} · <RelativeTime value={event.requested_at} /></span></div><Money currency={event.currency} minorUnits={event.amount_minor} /></div><p>{event.status === "requested" ? "Check the reference and supporting document before this can change a balance." : event.status === "approved" ? "The review is complete. Confirm the record again before adding the money." : event.status === "rejected" ? "No balance changed." : "This exact amount is reflected in the account record."}</p><div className="simple-record-actions"><Link className="button secondary" href={`/funding/${encodeURIComponent(event.funding_event_id)}?return_to=${encodeURIComponent(returnTo)}`}>{event.status === "requested" ? "Review now" : "View record"}</Link><TechnicalDetails><RecordIdentity label="Funding reference" value={event.funding_event_id} /><p>External reference: {event.external_reference}.</p></TechnicalDetails></div></article></li>)}</ul>{nextHref && <div className="pagination"><span>More records are available</span><Link className="button secondary" href={nextHref}>Next page</Link></div>}</section>}
    {events.length > 0 && mode === "expert" && <section className="ledger-section funding-ledger" aria-labelledby="funding-ledger-heading" aria-busy={loading}>
    ...
    {mode === "expert" && (events.length > 0 || Boolean(filters.status)) && <>
  ...
  return <>
    ...
    <section className="funding-docket" aria-labelledby="funding-docket-heading">
      ...
      <ol className="funding-evidence-rail">
        <FundingStage sequence="01" state="complete" title="Reference recorded" meta={<Timestamp value={event.requested_at} />}><dl><div><dt>Supporting reference</dt><dd>{event.evidence_reference}</dd></div>{mode === "expert" && <div><dt>Requester</dt><dd>{event.requester_subject_id}</dd></div>}{mode === "expert" && <div><dt>Event ID</dt><dd><CopyControl value={event.funding_event_id} /></dd></div>}</dl></FundingStage>
        ...
        <FundingStage sequence="03" state={journalComplete ? "complete" : event.status === "rejected" ? "blocked" : reviewComplete ? "current" : "blocked"} title={journalComplete ? "Balance updated" : "Balance unchanged"} meta={journalComplete ? "The exact amount was recorded." : "No money has been added yet."}><dl>{mode === "expert" && <div><dt>Destination version</dt><dd>{event.balance_version ? `v${event.balance_version}` : "Not available before posting"}</dd></div>}{mode === "expert" && <div><dt>Clearing identity</dt><dd>{event.system_account_id ? <CopyControl value={event.system_account_id} label="Copy funding clearing account ID" /> : "Pending"}</dd></div>}</dl></FundingStage>
      </ol>
    ...
    {mode === "simple" && <TechnicalDetails summary="View technical details"><RecordIdentity label="Funding reference" value={event.funding_event_id} />{event.journal_transaction_id && <RecordIdentity label="Journal reference" value={event.journal_transaction_id} />}<p>Requester: {event.requester_subject_id}.</p>{event.balance_version && <p>Balance version: {event.balance_version}.</p>}</TechnicalDetails>}
