...
export function EvidenceExportControl({label,subject,endpoint,scope,filters,columns,online,canExport}:Readonly<{label:string;subject:string;endpoint:string;scope:string;filters:ExportDisclosure[];columns:string;online:boolean;canExport:boolean;}>) {
  ...
  function download(event:FormEvent){
    ...
    if(!online){setError("The browser is offline. Reconnect before generating a new evidence file.");setState("error");dialog.current?.close();return;}
  ...
  return <div className="evidence-export-control">
    ...
    <dialog ref={dialog} className="confirmation-dialog export-review-dialog" aria-labelledby={`${frameName}-heading`} aria-describedby={`${frameName}-description`} onClose={()=>{if(!busy){setState("idle");trigger.current?.focus();}}}>
      <form method="dialog" onSubmit={download}><p className="eyebrow">Exact evidence export</p><h2 ref={dialogHeading} tabIndex={-1} id={`${frameName}-heading`}>Review {subject} export</h2><p id={`${frameName}-description`}>Confirm the authorized scope before generating a bounded, spreadsheet-safe evidence file.</p>
        ...
        <div className="export-not-backup"><WarningCircle weight="fill" aria-hidden="true"/><p><strong>This export is not a backup.</strong> It contains bounded operational evidence, not database state, restore metadata, credentials, or consistency tokens.</p></div>
