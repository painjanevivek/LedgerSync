...
export type CompletedAuthorization = Readonly<{
  identity: VerifiedIdentity;
  returnTo: string;
}>;
type OperatorAuthorization = Readonly<{
  tenantId: string;
  roles: string[];
  scopes: string[];
}>;
...
export function resolveOperatorAuthorization(
  subjectId: string,
  ...
  try {
    ...
    throw new Error("operator authorization mapping is invalid");
  }
  if (!parsed || Array.isArray(parsed) || typeof parsed !== "object")
    throw new Error("operator authorization mapping is invalid");
  const candidate = (parsed as Record<string, unknown>)[subjectId];
  if (!candidate || Array.isArray(candidate) || typeof candidate !== "object")
    throw new Error("operator is not invited");
        ...
        ...new Set(
          record.roles.filter(
            (value): value is string =>
              typeof value === "string" && allowedRoles.has(value),
          ),
        ),
      ]
    : [];
    ...
    ? [
        ...new Set(
          record.scopes.filter(
            (value): value is string =>
              typeof value === "string" && allowedScopes.has(value),
          ),
        ),
      ]
    : [];
...
function configuration() {
  ...
  if (!issuer || !clientID || !redirectURI)
    throw new Error(
      "OIDC issuer URL, client ID, and redirect URI are required",
    );
  if (
    process.env.NODE_ENV === "production" &&
    (!issuer.startsWith("https://") || !redirectURI.startsWith("https://"))
  )
  ...
  return {
    ...
    clientID,
    redirectURI,
    clientSecret: process.env.LEDGERSYNC_OIDC_CLIENT_SECRET,
  };
}
...
function sessionSecret() {
  ...
  if (!value || value.length < 32)
    throw new Error(
      "LEDGERSYNC_WEB_SESSION_SECRET must be at least 32 characters",
    );
  return value;
}
...
function randomURLValue(bytes = 32) {
...
export async function beginAuthorization(
  options: Readonly<{ prompt?: "login"; returnTo?: string }> = {},
): Promise<{ authorizationURL: string; transactionCookie: string }> {
  const config = configuration();
  const metadata = await discovery();
  const transaction: Transaction = {
    state: randomURLValue(),
    codeVerifier: randomURLValue(48),
    nonce: randomURLValue(),
  ...
  if (options.prompt === "login") url.searchParams.set("prompt", "login");
  return {
    authorizationURL: url.toString(),
    transactionCookie: signedTransaction(transaction),
  };
}
...
export async function completeAuthorization(
  ...
  const { payload, protectedHeader } = await jwtVerify(token.id_token, jwks, {
  ...
  return {
    identity: {
      ...
      ...authorization,
      expiresAt: Math.min(payload.exp * 1000, Date.now() + 30 * 60 * 1000),
      authenticatedAt,
    },
...
export function safeReturnTo(value: string | null | undefined): string {
  ...
  try {
    ...
    return parsed.origin === "https://ledgersync.invalid"
