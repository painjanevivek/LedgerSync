...
export async function proxyFundingMutation(
  session: Session,
  path: string,
  ...
  try {
    ...
    if (upstream.headers.get("idempotent-replay") === "true") response.headers.set("Idempotent-Replay", "true");
    const requestID = upstream.headers.get("x-request-id");
    if (requestID && /^[A-Za-z0-9._:-]{1,128}$/.test(requestID)) response.headers.set("X-Request-ID", requestID);
    const retryAfter = upstream.headers.get("retry-after");
    if (retryAfter && /^[0-9]{1,6}$/.test(retryAfter)) response.headers.set("Retry-After", retryAfter);
    return response;
  } catch (error) {
    return jsonError(isPrivateAPITimeout(error) ? "funding_outcome_unknown" : "temporary_unavailable", isPrivateAPITimeout(error) ? 504 : 503);
  }
}
