export type DeveloperOperation = Readonly<{ operation_id: string; method: "GET" | "POST" | "PUT" | "PATCH" | "DELETE"; path: string; scope: string }>;
...
const endpointGroupIDs = new Set(["accounts", "transfers", "corrections", "funding", "approvals", "reconciliation", "operations", "recovery_exports", "credentials", "webhooks", "developer"]);
const scopes = new Set(["accounts:read", "accounts:write", "transactions:read", "transfers:read", "transfers:write", "corrections:read", "corrections:write", "corrections:approve", "funding:read", "funding:write", "funding:approve", "reconciliation:read", "reconciliation:write", "local:read", "local:write", "events:read", "investigation:read", "investigation:write", "explainability:read", "developer:read", "credentials:read", "credentials:write", "webhooks:read", "webhooks:write", "webhooks:replay", "recovery:read", "exports:read"]);
...
function sanitizeOperation(value: unknown): DeveloperOperation | null {
  if (!isRecord(value) || !exactKeys(value, ["operation_id", "method", "path", "scope"])
    || !code(value.operation_id) || typeof value.method !== "string" || !["GET", "POST", "PUT", "PATCH", "DELETE"].includes(value.method)
...
function sanitizeGroup(value: unknown): DeveloperEndpointGroup | null {
  if (!isRecord(value) || !exactKeys(value, ["id", "label", "operations"]) || typeof value.id !== "string" || !endpointGroupIDs.has(value.id) || !bounded(value.label, 64) || !Array.isArray(value.operations) || value.operations.length < 1 || value.operations.length > 20) return null;
