...
const endpointGroupIDs = new Set(["accounts", "transfers", "reconciliation", "operations", "developer"]);
const scopes = new Set(["accounts:read", "accounts:write", "transactions:read", "transfers:read", "transfers:write", "reconciliation:read", "reconciliation:write", "local:read", "events:read", "developer:read"]);
...
function sanitizeMetadata(value: unknown): DeveloperMetadata | null {
  ...
  if (!Array.isArray(value.endpoint_groups) || value.endpoint_groups.length !== 5) return null;
  const groups = value.endpoint_groups.map(sanitizeGroup);
  if (groups.some((group) => group === null) || new Set(groups.map((group) => group?.id)).size !== 5) return null;
