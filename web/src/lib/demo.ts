...
export function createDemoSession(configuration: DemoConfiguration, now = Date.now()): Session {
  ...
  return {
    ...
    scopes: ["accounts:read", "accounts:write", "transactions:read", "transfers:read", "transfers:write", "reconciliation:read", "reconciliation:write", "local:read", "events:read", "explainability:read", "developer:read", "recovery:read", "exports:read"],
  };
}
