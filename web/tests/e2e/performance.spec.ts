...
async function installWebVitalObservers(page: Page) {
  await page.addInitScript(() => {
    const state = { lcpMilliseconds: 0, cls: 0, inpMilliseconds: 0, interactionCount: 0, shiftSources: [] as string[] };
...
test("compact throttled overview stays inside the pilot web-vital budgets", async ({ page, context }, testInfo) => {
  ...
  await testInfo.attach("compact-web-vitals.json", {
    body: Buffer.from(JSON.stringify({ profile: { viewport: "390x844", cpuThrottle: 4, network: "constrained-4g" }, initialLoad, observed }, null, 2)),
...
test("large bounded history progressively renders without blocking navigation", async ({ page }) => {
  await mockOperatorConsole(page);
