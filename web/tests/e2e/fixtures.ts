...
export async function mockOperatorConsole(page: Page) {
  await page.route("**/api/session", (route) => json(route, { subject_id: "operator-1", tenant_id: "tenant-1", csrf_token: "csrf-test-token", scopes: ["accounts:write", "transfers:write", "reconciliation:read", "reconciliation:write"], environment:"demo",tenant_label:"Meridian Labs · Test",operator_label:"Test operator" }));
